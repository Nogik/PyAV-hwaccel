...
cdef class HWConfig(object):
    ...
    def is_supported(self):
        return bool((self.ptr.methods & lib.AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX) or (self.ptr.methods & lib.AV_CODEC_HW_CONFIG_METHOD_HW_FRAMES_CTX))
...
cdef class HWAccel(object):
    ...
    def __init__(self, device_type=None, device=None, codec_pix_fmt=None, options=None, **kwargs):
        self._device_type = str(HWDeviceType.get(device_type)) if device_type else None
        self._device = device
    ...
    def create(self, Codec codec):
        return HWAccelContext(self._device_type, self._device, self.codec_pix_fmt, self.options, codec)


cdef class HWAccelContext(HWAccel):

    def __init__(self, device_type=None, device=None, codec_pix_fmt=None,  options=None, codec=None, **kwargs):
        super().__init__(device_type, device, codec_pix_fmt, options, **kwargs)
        ...
        for config in codec.hardware_configs:
            if not ((config.ptr.methods & lib.AV_CODEC_HW_CONFIG_METHOD_HW_DEVICE_CTX) or (config.ptr.methods & lib.AV_CODEC_HW_CONFIG_METHOD_HW_FRAMES_CTX)):
                continue
